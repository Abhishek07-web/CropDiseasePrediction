...
# Patch GlorotUniform deserialization compatibility for legacy H5 models.
def patch_glorot_uniform():
    try:
        import keras
    except ImportError:
        keras = None
    ...
    def patch_class(klass):
        if not klass or not hasattr(klass, '__init__'):
            return
        original_init = klass.__init__

        def __init__(self, seed=None, input_axes=None, output_axes=None, **kwargs):
            return original_init(self, seed=seed)

        klass.__init__ = __init__

    patch_class(tf.keras.initializers.GlorotUniform)
    if keras is not None and hasattr(keras, 'initializers') and hasattr(keras.initializers, 'GlorotUniform'):
        patch_class(keras.initializers.GlorotUniform)

patch_glorot_uniform()
...
def load_model_and_classes(model_path, class_indices_path):
    ...
    model = tf.keras.models.load_model(model_path, custom_objects={"GlorotUniform": GlorotUniformCompat})
